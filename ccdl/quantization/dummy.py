...
def fake_topk_quantize(x: torch.Tensor, group_size, bit, topk, stochastic=False, output: torch.Tensor = None) -> torch.Tensor:
    ...
    x[torch.arange(0, x.size(0), device=x.device)[:, None].expand(-1, topk), xabs_topk_index] = 0
    scale = x.abs().max(dim=-1).values.clamp_min(1e-6)
    ...
    x *= (2 ** (bit - 1) - 1) / scale[:, None]
    if stochastic:
        x += torch.rand_like(x) - 0.5
    x = x.round().clamp(-2 ** (bit - 1), 2 ** (bit - 1) - 1).to(dtype=torch.int8)
...
def fake_topk_dequantize(x: torch.Tensor, group_size, bit, topk, dtype, output: torch.Tensor = None, reduce_op = None) -> torch.Tensor:
    ...
    if dtype in ["fp16", "bf16"]:
        bytes_per_group = group_size + 2 + 4 * topk
    else:
        bytes_per_group = group_size + 4 + 8 * topk

    num_group = x.numel() // bytes_per_group
    x_ed = num_group * group_size
    ...
    _x = x[:x_ed].clone().view(dtype=torch.int8).view(-1, group_size)
    _scale = x[x_ed:scale_ed].clone().view(dtype=torch_dtype).view(-1)
    if topk > 0:
        _topk_index = x[scale_ed:topk_index_ed].clone().view(dtype=index_torch_dtype).to(torch.int64).view(-1, topk)  
        _top_values = x[topk_index_ed:].clone().view(dtype=torch_dype).view(-1, topk)
