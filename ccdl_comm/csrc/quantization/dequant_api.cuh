...
bool inplace_dequantize_reduce_mean_update_error_feedback(std::vector<torch::Tensor> inputs, torch::Tensor prepared, torch::Tensor restored, torch::Tensor residual, int64_t group_size, int64_t topk, int64_t bit, QuantType quant_type = QuantType::Linear, bool compact=false, int64_t divisor=1);
bool inplace_dequantize_reduce_update_local_error_feedback(std::vector<torch::Tensor> inputs, int64_t local_input_index, torch::Tensor prepared, torch::Tensor restored, torch::Tensor residual, int64_t group_size, int64_t topk, int64_t bit, QuantType quant_type = QuantType::Linear, bool compact=false, int64_t divisor=1);
