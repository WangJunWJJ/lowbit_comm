...
独立 checkpoint 质量评估必须使每个验证窗口全局恰好出现一次，保留不满 batch 的尾部，
允许局部空 rank，但拒绝全局空验证集。只有真实的全样本 batch-mean loss 才能按 batch 大小
加权；RS-IMLE 等按有效样本归一化的目标必须观察真实分子与有效分母，跨 rank 汇总后再相除，
并分别报告唯一窗口覆盖数、有效/无效窗口数及明确标识的全窗口零填充均值。禁止取 rank 均值
的简单平均，也禁止从标量 loss 和 batch 大小猜测有效分母；全局有效分母为零必须失败。
实际索引、样本计数、所有 rank 身份必须与无补齐分片一致；非有限损失或不一致的
记录不得产生成功结果。评估必须绑定原训练结果/协议、checkpoint、输入清单及评估代码散列，
不得覆盖旧结果或把离线评估耗时混入原训练性能。索引互斥不等于场景独立，重新分批也会改变
随机损失的采样及原有batch相关筛选；新旧损失不能冒充单因素 padding 消融。单独评估不授予 adapter 生产资格。
