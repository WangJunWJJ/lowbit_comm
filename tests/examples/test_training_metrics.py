...
def test_result_contains_stable_timing_correctness_and_execution_schema() -> None:
    ...
    assert payload["timing"]["p50_step_latency_ms"] == 200.0
    assert payload["timing"]["p95_step_latency_ms"] == 210.0
    assert payload["timing"]["overlap_efficiency"] == 0.5
    assert payload["timing"]["communication_ms"] == 4.0
    assert payload["timing"]["overlap_classification"] == "timeline_overlapped"
